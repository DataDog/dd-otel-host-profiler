...
func NewElfFromDisk(path string) (*Elf, error) {
	...
	if err != nil {
		log.Debug("failed to get GNU build ID for file", log.String("path", path), log.String("error", err.Error()))
	}

	if wrapper.elfFile.IsGolang() {
		goBuildID, err = wrapper.elfFile.GetGoBuildID()
		if err != nil {
			log.Debug("failed to get Go build ID for file", log.String("path", path), log.String("error", err.Error()))
		}
	}
...
func (e *Elf) GetSize() int64 {
	...
	if elfPath == "" {
		...
		if err != nil {
			log.Warn("Failed to get elf data", log.String("error", err.Error()))
			return 0
		}
	...
	if err != nil {
		log.Warn("Failed to get elf file", log.String("error", err.Error()))
		return 0
	}
	return fi.Size()
}
